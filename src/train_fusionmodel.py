...
def initModels():
    ...
    torch.load('/content/drive/MyDrive/app/sesface_1/sesface_backup_epoch13.pth')['net'])
...
def eval():
    ...
    #    net, optimizer, last_epoch, scheduler = common_init(args)
    net = fusion_model.FusionModel()
    net.load_state_dict(torch.load(args.model_file))  # 加载模型可学习参数
    net.to(args.device)
    acc = val.fusion_val2(-1, 16, 64, args.device, srnet, fnet, lr_fnet, net)
    acc = val.fusion_val2(-1, 8, 64, args.device, srnet, fnet, lr_fnet, net)
    acc = val.fusion_val2(-1, 4, 64, args.device, srnet, fnet, lr_fnet, net)
