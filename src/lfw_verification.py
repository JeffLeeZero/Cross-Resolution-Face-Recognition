...
def run(args, fnet, net=None, step=None):
    fnet.eval()
    if net is not None:
        net.eval()
    ...
    dataloader = lfw_loader.get_loader(args)
    ...
    with torch.no_grad():
        ...
        for index, (img1, img2, img1_flip, img2_flip, targets) in enumerate(tqdm(dataloader, ncols=0)):
            ...
            features21, _ = net(img2)
            features22, _ = net(img2_flip)
    ...
    if args.size != -1:
        ...
        message += '(down_factor:{} {}x{})'.format(args.down_factor, round(112 / args.down_factor),
                                                   round(96 / args.down_factor))
